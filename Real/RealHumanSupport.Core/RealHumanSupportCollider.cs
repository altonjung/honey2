...
{
    public class RealHumanSupportCollide
    {
        private const string RealHumanColliderPrefix = "Cloth colliders support_";
        ...
        internal static void AllocateClothColliders(RealHumanData humanData, string xml, string clothName, string uniqueId, Cloth[] clothes, bool isTop)
        {
            if (humanData == null || humanData.chaCtrl == null || string.IsNullOrEmpty(xml) || clothes == null || clothes.Length == 0)
                return;
            ...
            foreach (XElement cloth in root.Elements("cloth"))
            {
                foreach (XElement element in cloth.Elements())
                {
                    if (element.Name == "CapsuleCollider")
                    {
                        ...
                        if (capsule != null && !ContainsCapsuleData(humanData.capsuleColliders, capsule))
                            humanData.capsuleColliders.Add(capsule);
                    }
                    else if (element.Name == "SphereColliderPair")
                    {
                        ...
                        if (!ContainsSpherePairData(humanData.sphereColliders, pair))
                            humanData.sphereColliders.Add(pair);
                    }
                }
            }

            UpdateClothColliders(humanData, clothes, isTop);
        }

        internal static void UpdateClothColliders(RealHumanData humanData, Cloth[] targets, bool isTop)
        {
            if (humanData == null || humanData.chaCtrl == null || targets == null || targets.Length == 0)
                return;

            string targetClothName = isTop ? ClothTopName : ClothBottomName;
            ChaControl chaCtrl = humanData.chaCtrl;

            List<SphereColliderPair> sphereData = humanData.sphereColliders
                .Where(v => v != null && string.Equals(v.ClothName, targetClothName, StringComparison.OrdinalIgnoreCase))
                .ToList();

            List<CapsuleColliderData> capsuleData = humanData.capsuleColliders
                .Where(v => v != null && string.Equals(v.ClothName, targetClothName, StringComparison.OrdinalIgnoreCase))
                .ToList();
        ...
        private static SphereCollider AddSphereCollider(ChaControl chaCtrl, SphereColliderData sphereColliderData)
        {
            ...
            string colliderName = $"{RealHumanColliderPrefix}_{sphereColliderData.BoneName}";
        ...
        private static CapsuleCollider AddCapsuleCollider(ChaControl chaCtrl, CapsuleColliderData capsuleColliderData)
        {
            ...
            string colliderName = $"{RealHumanColliderPrefix}_{capsuleColliderData.BoneName}";
